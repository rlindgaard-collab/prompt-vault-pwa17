...
export default function App(){
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem("pv531_tab") || "");
  const [category, setCategory] = useState(() => localStorage.getItem("pv531_cat") || "");
  const [q, setQ] = useState("");
  const [count, setCount] = useState(0);
  const [copiedIdx, setCopiedIdx] = useState(null);

  useEffect(()=>localStorage.setItem("pv531_tab", activeTab),[activeTab]);
  useEffect(()=>localStorage.setItem("pv531_cat", category),[category]);
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-200">
        <div className="max-w-6xl mx-auto p-3 flex items-center justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold">Prompt Vault v5.3.1</h1>
